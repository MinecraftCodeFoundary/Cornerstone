...
public final class CornerstoneLifecycleEvents {
    ...
    @SubscribeEvent public static void onPlayerLoggedIn(PlayerEvent.PlayerLoggedInEvent event) {
        if (event.getEntity() instanceof net.minecraft.server.level.ServerPlayer player
                && (!Config.moduleEnabled("moderation") || !Cornerstone.services().moderation().disconnectIfBanned(player))
                && (!Config.moduleEnabled("operations") || !Cornerstone.services().maintenance().disconnectIfBlocked(player))) Cornerstone.services().lifecycle().playerJoined(player);
    }
    ...
    @SubscribeEvent public static void onPlayerTick(PlayerTickEvent.Post event) {
        if (event.getEntity() instanceof net.minecraft.server.level.ServerPlayer player) { if (Config.moduleEnabled("moderation")) Cornerstone.services().moderation().enforceFreeze(player); if (Config.moduleEnabled("social")) Cornerstone.services().social().playerTick(player); }
    }
    @SubscribeEvent public static void onChat(ServerChatEvent event) {
        if (Config.moduleEnabled("moderation") && Cornerstone.services().moderation().isMuted(event.getPlayer().getUUID())) { event.setCanceled(true); event.getPlayer().sendSystemMessage(net.minecraft.network.chat.Component.literal("You are muted.")); return; }
        if (Config.moduleEnabled("social")) event.setMessage(Cornerstone.services().social().formatChat(event.getPlayer(), event.getRawText()));
    }
